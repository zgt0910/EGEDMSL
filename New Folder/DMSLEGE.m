function [Z,G] = DMSLEGE(X,k,lambda,label) 
    ...
    maxIter=20;
    afa=0.0001;%z前面系数
    beta=0.0001;%e前面系数取0.00001到0.001
%     lambda3=0.001;%r前面系数
    gama=lambda;%Zk前面系数0.00001或0.001
    miu = 0.01;
    invmiu=1/miu;
    rho = 1.1;
    max_miu = 1e8;
 ...
 %% update P
    ...
    L2=W-invmiu*A3;
    IM=eye(m);
    % [Sw,Sb] = LDA( X',label,num_class);%输入保证为n*d
    LZ=diag(Z*ones(n,1))-(Z);
    %  P=pinv(X*LS*X'+miu*X*Z*Z'*X')*(miu*X*Z*M6'*R);
    P=(X*LZ*X'+SE+miu*X*Z*Z'*X'+miu*IM)\(miu*X*Z*L1'+miu*L2);
    %     P=(eye(m)/(X*LS*X'+miu*X*Z*Z'*X'))*(miu*X*Z*M6'*R);
    %% update Z
    Zk = Z;
    L3=G-invmiu*A4;
    I = eye(n);
