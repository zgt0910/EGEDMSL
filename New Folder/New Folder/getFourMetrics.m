function results = getFourMetrics(labels,true_labels)
% Huang Dong. 20160904.
...
result = ClusteringMeasure(true_labels,labels);
CA = result(1);
purity = result(3);
[ARI,RI,MI,HI]=valid_RandIndex(labels,true_labels);
NMI = NMImax(labels,true_labels);

results = [CA,NMI,purity,ARI];
